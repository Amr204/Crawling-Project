...
# تأكد من تنزيل الموارد اللازمة من NLTK (قم بتشغيل السطرين التاليين مرة واحدة إذا لم تقم بذلك مسبقًا)
# nltk.download('punkt')
...
def start_crawling():
    ...
    try:
        ...
        max_tasks = int(tasks_entry.get())
        strategy_val = int(strategy_entry.get())
    except ValueError:
        messagebox.showerror("خطأ", "يرجى التأكد من إدخال قيم رقمية صحيحة في الحقول المناسبة")
        return

    # إنشاء كائن args باستخدام argparse.Namespace لتوافق مع الدالة main
    args = argparse.Namespace(start_url=start_url, max_depth=max_depth, max_tasks=max_tasks, strategy=strategy_val)

    # تشغيل عملية الزحف في خيط منفصل حتى لا يتجمد واجهة المستخدم
    threading.Thread(target=lambda: asyncio.run(main(args)), daemon=True).start()
    messagebox.showinfo("بدأ الزحف", "تم بدء عملية الزحف.\nيمكنك متابعة السجل في ملف crawler.log")

# إنشاء نافذة الواجهة باستخدام Tkinter
root = tk.Tk()
root.title("واجهة الزاحف")

# مكون لإدخال الرابط الابتدائي
tk.Label(root, text="الرابط الابتدائي:").grid(row=0, column=0, padx=5, pady=5, sticky="e")
url_entry = tk.Entry(root, width=50)
url_entry.grid(row=0, column=1, padx=5, pady=5)
url_entry.insert(0, "https://ust.edu.ye")

# مكون لإدخال أقصى عمق للزحف
tk.Label(root, text="أقصى عمق:").grid(row=1, column=0, padx=5, pady=5, sticky="e")
depth_entry = tk.Entry(root, width=10)
depth_entry.grid(row=1, column=1, padx=5, pady=5, sticky="w")
depth_entry.insert(0, "1")

# مكون لإدخال عدد الطلبات المتزامنة
tk.Label(root, text="عدد الطلبات المتزامنة:").grid(row=2, column=0, padx=5, pady=5, sticky="e")
tasks_entry = tk.Entry(root, width=10)
tasks_entry.grid(row=2, column=1, padx=5, pady=5, sticky="w")
tasks_entry.insert(0, "10")

# مكون لاختيار استراتيجية الزحف: 1 لـ BFS، 2 لـ Greedy، 3 لـ DFS
tk.Label(root, text="استراتيجية الزحف (1: BFS, 2: Greedy, 3: DFS):").grid(row=3, column=0, padx=5, pady=5, sticky="e")
strategy_entry = tk.Entry(root, width=10)
strategy_entry.grid(row=3, column=1, padx=5, pady=5, sticky="w")
strategy_entry.insert(0, "1")

# زر لبدء عملية الزحف
start_button = tk.Button(root, text="ابدأ الزحف", command=start_crawling)
